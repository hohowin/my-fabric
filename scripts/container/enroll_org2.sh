...
fabric-ca-client register -d --id.name peer0.gov.hk --id.secret 4PBPEkwt --id.type peer -u https://0.0.0.0:6055
fabric-ca-client register -d --id.name peer1.gov.hk --id.secret Q528SdnZ --id.type peer -u https://0.0.0.0:6055
fabric-ca-client register -d --id.name admin-gov.hk --id.secret mEN6bpQW --id.type admin --id.attrs "hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert,abac.init=true:ecert" -u https://0.0.0.0:6055
...
fabric-ca-client enroll -d -u https://peer0.gov.hk:4PBPEkwt@0.0.0.0:6055
...
fabric-ca-client enroll -d -u https://peer1.gov.hk:Q528SdnZ@0.0.0.0:6055

# Enroll HKGov's Admin
export FABRIC_CA_CLIENT_HOME=/var/artifacts/crypto-config/HKGovMSP/admin
export FABRIC_CA_CLIENT_TLS_CERTFILES=/var/artifacts/crypto-config/HKGovMSP/peer0.gov.hk/assets/ca/gov.hk-ca-cert.pem
export FABRIC_CA_CLIENT_MSPDIR=msp
fabric-ca-client enroll -d -u https://admin-gov.hk:mEN6bpQW@0.0.0.0:6055
